...
METRICS = ['BRISQUE', 'BSNR', 'ERGAS', 'FSIM', 'GMSD', 'HAARPSI', 'MAD',
        'MSE', 'MSSSIM', 'NIQE', 'NQM','NRMSE', 'PAMSE', 'PSNR', 'RASE',
        'SAM', 'SCC', 'SNR', 'SSIM', 'UQI', 'VIF', 'WSNR']
...
def GetFileList(path, extentions, remove_extension=True, full_path = False):
    """
    ...
    for ext in extentions:
        dosyalar= dosyalar + glob.glob(join(path, ext))

    dosyalar = list(dict.fromkeys(dosyalar))  # remove duplicates

    if full_path:
        if remove_extension:
            return [join(path, y) for y in [splitext(x) for x in dosyalar]]
        else:
            return [join(path, y) for y in dosyalar]
    else:
        if remove_extension:
            files = [splitext(basename(x))[0] for x in dosyalar]
        else:
            files = [basename(x) for x in dosyalar]
...
def measure_images(image_path, scale=2, metrics= "ALL", channels= 1, decimation="bicubic",
                   methods= METHODS, border=0, pad=0, save_images=False):
    """
    ...
    """
    file_list = GetFileList(image_path, image_extentions, remove_extension=False, full_path=True)
    ...
    #for file in file_list:
        ...
        if decimation != "same":
            ...
            im_b = Preprocess_Image(im_g, scale=scale, pad=pad, channels=channels,
        ...
        for method in methods:
            ...
            im_m = imresize(im_b, scale / 1.0, interp= method)  # upscaled image with given method
...
def Preprocess_Image(img, scale=2, pad=0, channels=1, upscale=False, crop_remains=True,
                     img_type='input', interp_down='bicubic', interp_up='bicubic') :
    """
    Prepares image as ground truth, bicubic or downscaled image for model

    Parameters:
        img : image to pre-process
        scale    : the scale at which image is downscaled and, or, upscaled
        pad     :  the size of pixels cropped from each borders of image
        channels : processing channel. for example, Y channel (first) of YCbCr color space.
                    set 3 for all channels
        normalize : image is divided by 255.0 if set True
        upscale    : image is upscaled after downscaling, is set True
        crop_remains : The image is cropped to fit the exact fold size of scale, if set.
                        the remaining number pixels result in (image shape size) % scale
        img_type    : One of ['input', 'bicubic' , 'ground']. they indicates if the image is processed is the image to be input to model
                        or bicubic scaled image, or ground truth image. The image is processed accordingly
        subtract_mean_image: Indicates whether the overall mean of images to be subtracted from the image or not.
        mean: The mean image of overall images in the training set.
        interpolation   : The interpolation method. Onne of 'bicubic', 'nearest', 'lanczos', 'cubic'. Default is 'bicubic'
    Results:
    ...
    # as is in the parameter 'interp_down'
    if interp_up is None:
        interp_up = interp_down
    ...
    img = misc.imresize(img, 1.0/scale, interp=interp_down) #type uint8

    if upscale:
        img = misc.imresize(img, scale / 1.0, interp= interp_up) # bicubic interpolated image
...
def Prepare_Input_Ground_Bicubic(img, scale=2, pad=0, upscaleimages=True, channels=1):
    ...
    normalize -> if normalize is True, intensity values of image is divided by 255.
...
def Prepare_Image_For_Model_Input(img_input, channels=1, ordering_style='tf'):
    """
...
def Test_Single_Image(model, input_image, border=0, modelscalesimage=True,
    ...
    if normalize:
        ...
        img_input = Prepare_Image_For_Model_Input(img_input, channels)
    ...
    psnr_model, ssim_model = \
        calc_metrics_of_image(img_ground, img_result, border, calcchannels)

    psnr_bicubic, ssim_bicubic = \
        calc_metrics_of_image(img_ground, img_bicubic, border, calcchannels)
...
def shuffle_data(output_file):
    """
    Shuffles the input images and input image patches.
    :param output_file:
    :return:
    """
